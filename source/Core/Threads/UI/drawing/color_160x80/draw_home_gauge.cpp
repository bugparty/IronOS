...
void drawTemperatureNumber(TemperatureType_t temperature, uint8_t x, uint8_t y, uint8_t color) {
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    Display::drawBitmapColor(Hs02InterSemiBoldFont::kDigits[digits[i]], Hs02InterSemiBoldFont::kDigitWidth,
                             Hs02InterSemiBoldFont::kDigitHeight, x + i * Hs02InterSemiBoldFont::kDigitWidth, y, color);
  }
}
...
uint8_t drawWashText(const char *text, uint8_t x, uint8_t y, uint8_t color) {
  while (*text) {
    ...
    const int8_t index = Hs02WashFont::glyphIndex(character);
    if (index >= 0) {
      Display::drawBitmapColor(Hs02WashFont::kGlyphs[index], Hs02WashFont::kGlyphWidth, Hs02WashFont::kGlyphHeight,
                               x - washGlyphLeftBearing(character), y, color);
    }
...
uint8_t drawWashTenthsEmphasis(uint32_t value, uint8_t x, uint8_t y, uint8_t color, char unit) {
  ...
  for (uint8_t i = 0; i < count; ++i) { reversed[i] = '0' + whole % 10; whole /= 10; }
  while (count) { text[pos++] = reversed[--count]; }
  text[pos++] = '.';
  text[pos++] = '0' + value % 10;
  text[pos++] = unit;
...
void drawSleepCountdown() {
  ...
  for (uint8_t i = digits; i > 0; --i) { number[i - 1] = '0' + remaining % 10; remaining /= 10; }
